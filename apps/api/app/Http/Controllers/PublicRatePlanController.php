...
class PublicRatePlanController extends Controller
{
    public function index(Request $request, string $slug, ManualQuoteSource $source): JsonResponse
    {
        ...
        foreach (explode('&', (string) $request->server('QUERY_STRING', '')) as $pair) {
            ...
            abort_if(! in_array($key, ['arrival', 'departure', 'adults', 'currency'], true) || isset($seen[$key]), 422, 'Unsupported or repeated selection parameter.');
            $seen[$key] = true;
        }
        $selection = $request->validate([
            'arrival' => ['required', 'date_format:Y-m-d'], 'departure' => ['required', 'date_format:Y-m-d', 'after:arrival'],
            'currency' => ['sometimes', 'required', Rule::in(['LKR', 'USD'])],
            'adults' => ['required', 'integer', 'between:1,30'],
        ]);
        $selection['currency'] = $selection['currency'] ?? 'LKR';
        abort_if(array_diff(array_keys($request->query()), ['arrival', 'departure', 'adults', 'currency']) !== [], 422, 'Unsupported search fields.');
        ...
        return DB::transaction(function () use ($slug, $selection, $source): JsonResponse {
            ...
            foreach (RatePlan::where('hotel_id', $hotel->id)->where('status', 'active')->where('currency', $selection['currency'])->orderBy('id')->get() as $plan) {
                $quote = $source->resolve(array_diff_key($selection, ['currency' => true]) + ['hotel_id' => $hotel->id, 'rate_plan_id' => $plan->id], $now);
                if ($quote === null) {
                    continue;
                }
                ...
                $offers[] = ['rate_plan_id' => $plan->id, 'name' => $plan->name, 'room_type_id' => $room->id, 'room_name' => $room->name,
                    'max_adults' => $room->max_occupancy, 'currency' => $plan->currency, 'meal_plan' => $plan->meal_plan, 'total_minor' => $total, 'policy' => $quote['input']['policy'],
