...
function RoomConfiguration({ hotel, room, step }: { hotel: Hotel; room: Room; step: string }) {
  ...
  return <><label>Rate plan<select value={chosen} onChange={e => { if (canLeaveOperations()) setChosen(e.target.value === "new" ? "new" : Number(e.target.value)); }}><option value="new">New rate plan</option>{snapshot.data.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
    <OperationForm key={`${chosen}:${plan?.version}`} label={plan ? "Save rate plan" : "Create rate plan"} editable={hotel.permissions.manage_inventory} reload={load} save={async data => { const result = await request<{ data: Plan }>(`${root}/rate-plans${plan ? `/${plan.id}` : ""}`, plan ? "PUT" : "POST", { name: text(data, "name"), status: text(data, "status"), currency: "LKR", policy: { version: text(data, "policy_version"), text: text(data, "policy_text") }, ...(plan ? { version: plan.version } : {}) }); setChosen(result.data.id); await load(); }}>
      <Field label="Rate plan name" name="name" defaultValue={plan?.name} required maxLength={255} /><Status value={plan?.status} /><Field label="Policy revision label" name="policy_version" defaultValue={plan?.policy.version} required maxLength={100} /><p>Currency: LKR</p><label className="full">Cancellation and rate policy<textarea name="policy_text" defaultValue={plan?.policy.text} required maxLength={10000} rows={5} /></label>
    </OperationForm></>;
}
