...
export default function PasswordSetup() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const message = newPasswordError(String(values.get("password") || ""));
    if (message) { setError(message); event.currentTarget.querySelector<HTMLInputElement>('input[name="password"]')?.focus(); return; }
    setBusy(true); setError("");
    try { await api("password/setup", "POST", { ...Object.fromEntries(values), ...details }); window.history.replaceState(null, "", window.location.pathname); setDone(true); }
  ...
  return <main className="signin"><Link className="wordmark" href="/admin">niwadu<span>hotel management</span></Link><h1>{done ? "Your password is ready" : "Set your password"}</h1>
    {done ? <><p>Your account is ready to use.</p><Link className="button-link" href="/admin">Sign in</Link></> : <><p id="new-password-help">{NEW_PASSWORD_HELP}</p>{details && <form className="stack" onSubmit={submit}><p>{details.email}</p><label>New password<input type="password" name="password" aria-describedby="new-password-help" minLength={12} maxLength={1024} autoComplete="new-password" required /></label><label>Confirm password<input type="password" name="password_confirmation" minLength={12} maxLength={1024} autoComplete="new-password" required /></label><button disabled={busy}>{busy ? "Saving…" : "Set password"}</button></form>}</>}
