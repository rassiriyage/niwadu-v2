    ...
    <dialog className="discovery-dialog" ref={dialog} aria-labelledby="filter-title">
      <form ref={form} action="/hotels" method="get">
        ...
        <fieldset><legend>Property type</legend><p>Match any selected type.</p>
          {options.property_types.length ? options.property_types.map(type => <label key={type.key}><input type="checkbox" name="property_types[]" value={type.key} defaultChecked={query.types.includes(type.key)} />{type.label}</label>) : <p>No property types are available yet.</p>}
        </fieldset>
