...
async function request<T>(path: string, method = "GET", data?: unknown, token?: string): Promise<T> {
  let response: Response;
  try {
    response = await fetch(`/api/v1/${path}`, {
      method, credentials: "same-origin", cache: "no-store", signal: AbortSignal.timeout(15000),
      headers: { Accept: "application/json", ...(token ? { "X-CSRF-TOKEN": token, "Content-Type": "application/json" } : {}) },
