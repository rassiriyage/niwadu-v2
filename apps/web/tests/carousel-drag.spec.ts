...
test('QA image drag scrolls the carousel without activating a card',async({page})=>{
 ...
 const track=page.locator('.card-track').first();const image=track.locator('.card-image img').nth(2);
...
test('QA drag on wishlist does not activate it; fresh clicks and arrows still work',async({page})=>{
 await page.setViewportSize({width:1440,height:900});await page.goto('/');const track=page.locator('.card-track').first();
 const heart=track.locator('.wishlist').nth(2);const b=(await heart.boundingBox())!;
 ...
 const previous=page.getByRole('button',{name:'Previous Popular stays in Nuwara Eliya',exact:true});await previous.click();
...
test('QA release outside the row stops dragging and keeps fresh clicks working',async({page})=>{
 await page.setViewportSize({width:1440,height:900});await page.goto('/');const track=page.locator('.card-track').first();const b=(await track.locator('.card-image').nth(2).boundingBox())!;
