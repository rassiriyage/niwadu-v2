...
test("reference homepage stays local and honest", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Destination inspiration", exact: true })).toBeVisible();
  ...
  expect(hrefs.filter(href => !allowed.has(href) && !/^\/hotels\/[^/?#]+$/.test(href) && !href.startsWith("#") && !href.startsWith("tel:") && !href.startsWith("mailto:")), "Unexpected homepage navigation targets").toEqual([]);
  ...
  const track = page.getByRole("list", { name: "Destination inspiration listings", exact: true });
  await page.getByRole("button", { name: "Next Destination inspiration", exact: true }).click();
  await expect.poll(() => track.evaluate(el => el.scrollLeft)).toBeGreaterThan(0);
  for (const trigger of [page.locator("#destination-inspiration .listing-card").first(), page.locator(".footer-columns button").first()]) {
    await trigger.click();
...
test("public photos use responsive optimized delivery with one priority image", async ({ page }) => {
  ...
  await expect(page.locator(".photo-slot")).toHaveCount(7);
  expect(await page.locator(".photo-slot > img").count()).toBeLessThan(7);
  ...
  const first = content.find(section => section.kind === "destination")!.cards[0];
