...
## Expanded classification fixture

After the expanded candidate's guarded PG suite, run `php artisan db:seed --class=ClassificationTestSeeder --force` with the same isolated environment/PHP8.4 above. It invokes guarded discovery seeder and produces two explicit synthetic reviewed classifications: qa-fixture-villa => destination synthetic-coast, district galle, themes beach/adventure, amenities wifi/pool, editorial1; qa-fixture-hotel => synthetic-city, trincomalee, city, wifi/parking, editorial10. Stars remain null. This fixture does not create manual rate rows; if combining with dated rates, run ManualCatalogTestSeeder FIRST then ClassificationTestSeeder because discovery seeding resets metadata snapshots. Use fresh actual IDs from API. Run one DB owner at a time.

## Guided room setup fixture

The wizard candidate passed owner SQLite54/590 and PostgreSQL45/622. After reserving the isolated target and running the candidate's guarded PostgreSQL suite, seed with `php artisan db:seed --class=WizardRoomTestSeeder --force --no-interaction` using the environment/PHP8.4 above. This calls ManualCatalogTestSeeder, then creates independent BB LKR and USD offers on the same room/pool: nightly base1,100,000 LKR minor units or12,500 USD minor units, explicit zero tax/fee. Dates/selection IDs are printed; no currency conversion occurs. It also prints an owned legacy draft for `wizard-employee@example.test`, using DISCOVERY_FIXTURE_PASSWORD, for explicit conversion and wizard resume testing. Legacy indicative rate9999 is evidence only. Seeder was verified successfully; do not assume printed IDs remain stable after a test reset. For combined classification fixtures, run ClassificationTestSeeder afterward. Preserve BookingQuoteSource binding, UTC database configuration and current auth/session/migration tests when integrating this bounded Catalog delta.
