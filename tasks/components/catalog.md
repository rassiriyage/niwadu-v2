...
## Implemented discovery slice — 2026-09-24

Implemented sole discovery migration, administrator review/release/withdraw actions with source/release version fencing and audit, immutable reserved slug and metadata snapshot, public list/detail/options, strict query parsing, neutral photo:null and no-store responses. Draft metadata stays private until explicit approval; hotel status/bookability are unaffected. Six existing property type keys are shared in config. Verification: full SQLite 41 tests / 357 assertions; PostgreSQL 29 tests / 328 assertions including actual lock-barrier concurrency and migration rollback/reapply. Synthetic guarded seeder passed. See [catalog-discovery-qa.md](catalog-discovery-qa.md) for exact isolated setup; independent integration acceptance remains pending. No stock/hold schema or hosted release.
